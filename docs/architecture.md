...
make foreground, top-resumed, known blocker, fixed-`pidof` quality, and
separately parsed global `mCurrentFocus`/`mFocusedApp` facts structural, but PID
absence is not an application failure or readiness result. FocusPlaceholderActivity
is an observed system component, not a universal failure verdict, and
global-focus contradictions remain observations. QFM reports Android focus
observations only. The application owns panel-paused state, advancing focused/
submitted frames, the >=750 ms stability window, app-owned handoff markers,
OpenXR readiness, and interpretation. Application/OpenXR readiness remains
`unknown` with authority `false` until an app owner reports it. See
